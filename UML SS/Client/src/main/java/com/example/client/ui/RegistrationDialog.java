...
public class RegistrationDialog extends JDialog {
    ...
    public RegistrationDialog(JFrame parent, ApiClient apiClient, Long currentUserId) {
        super(parent, "Register with Mentor", true);
    ...
    private void initializeUI() {
        ...
        JLabel titleLabel = new JLabel("Select a Mentor to Register With");
    ...
    private void showConfirmationDialog(MentorDTO mentor) {
        JDialog confirmDialog = new JDialog(this, "Xác nhận đăng ký", true);
        ...
        JLabel purposeLabel = new JLabel("Mục đích đăng ký:");
        purposeLabel.setFont(new Font("Arial", Font.BOLD, 14));
        infoPanel.add(purposeLabel);
        infoPanel.add(Box.createVerticalStrut(5));
        
        JTextArea purposeArea = new JTextArea(6, 30);
        purposeArea.setFont(new Font("Arial", Font.PLAIN, 13));
        purposeArea.setLineWrap(true);
        purposeArea.setWrapStyleWord(true);
        JScrollPane purposeScroll = new JScrollPane(purposeArea);
        purposeScroll.setBorder(BorderFactory.createLineBorder(Color.GRAY));
        infoPanel.add(purposeScroll);
        ...
        confirmButton.addActionListener(e -> {
            ...
            if (purpose.isEmpty()) {
                JOptionPane.showMessageDialog(confirmDialog, 
                    "Vui lòng nhập mục đích đăng ký", 
                    "Thiếu thông tin", 
                    JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            submitRegistration(mentor.getId(), purpose);
        });
